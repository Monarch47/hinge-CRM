...
`typeText()` has one path: the line is folded to ASCII (homoglyphs mapped, unmappable chars dropped and logged) and typed with `input text` on whatever keyboard the phone already has selected. No IME is switched.

So **emoji and accents never survive** — write **text emoticons** instead: `:)` `:P` `:0` `:')` `;)` `:D`.
